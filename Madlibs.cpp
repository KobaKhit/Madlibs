...
int main(void)
{
    ...
    // Repeatedly prompts user for another filename if entered file does not exist or the name is
    // incorrect
    ifstream fin;
    fin.open(filename);
    while(fin.fail()){
        fin.clear();
        string prompt = "File does not exist. Try again:";
        string filename = promptForFilename(prompt);
        fin.open(filename);
    }
